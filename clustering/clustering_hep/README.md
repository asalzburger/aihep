...
The `clustering_hep` package is split into four submodules:

| submodule | contents |
|---|---|
| `clustering_hep.edm` | **Event data model**: the transient per-event table schemas (`hits`, `clusters`, `truth` column lists) shared by `sim`, `io`, and `vis` — the single source of truth for what a row of each table looks like. |
| `clustering_hep.sim` | Everything that produces an event: detector/particle configuration (`config.py`), track/pixel-grid geometry (`geometry.py`), particle generation (`simulate.py`), digitization — diffusion/noise/threshold (`digitize.py`), and connected-component cluster finding (`clustering.py`). |
| `clustering_hep.io` | Read/write `hits`/`clusters`/`truth` tables as CSV or Apache Arrow. |
| `clustering_hep.vis` | Matplotlib visualization: single-event display and cluster summary plots. |

`clustering_hep.cli` ties these together into the `run`/`visualize` commands
...
```bash
.venv/bin/python -m clustering_hep.cli run \
...
```bash
.venv/bin/python -m clustering_hep.cli visualize \
  --config configs/default.yaml \
  --output-dir out/ \
  --format arrow \
  --event-id 0 \
...
```python
from clustering_hep.io import read_run
from clustering_hep.vis import plot_cluster_summary
...
falls back to its default (see `clustering_hep/sim/config.py`).
...
The schemas below are defined once, in `clustering_hep/edm.py`, and reused
...
```python
from clustering_hep.sim import SimConfig
from clustering_hep.cli import run_simulation
